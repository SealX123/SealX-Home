	...
	function (a, b) {
		...
		a.cleanData = function (b) {
			...
			var e = d.call(arguments, 1),
				...
				h, i;
			for (; f < g; f++)
				for (h in e[f]) i = e[f][h], e[f].hasOwnProperty(h) && i !== b && (c[h] = a.isPlainObject(i) ? a.widget.extend({}, c[h], i) : i);
			return c
	...
	function (a, d) {
		...
		var e = a(b),
			...
			h = {
				...
				isFirstPageUrl: function (b) {
					var c = h.parseUrl(h.makeUrlAbsolute(b, q)),
		...
		x = function () {
			...
			if (g.length === 0)
				...
				else if (h.isEmbeddedPage(l)) return e.reject(k, c), e.promise();
			if (g.length) {
				if (!f.reloadPage) return B(g, f.role), e.resolve(k, c, g), e.promise();
				i = g
			}
			var n = f.pageContainer,
			...
			a(c).delegate("form", "submit", function (b) {
				...
				var k = f.is("[rel='external']") || f.is(":jqmData(ajax='false')") || f.is("[target]"),
					l = k || h.isExternal(j) && !h.isPermittedCrossDomainRequest(p, j);
				if (l) {
					g();
					return
				}
	...
	function (a, b) {
		...
		a(c).ready(a.proxy(j, "resolve")), a(c).one("mobileinit", a.proxy(i, "resolve")), a.extend(e, {
			...
			onHashChange: function (b) {
				...
				var d, e, g = a.mobile.path.parseLocation().hash,
	...
	function (a, b) {
		...
		a.fn.buttonMarkup = function (d) {
			...
			for (var h = 0; h < f.length; h++) {
				var i = f.eq(h),
					j = i[0],
					k = a.extend({}, a.fn.buttonMarkup.defaults, {
			...
			a(c).bind({
				"vmousedown vmousecancel vmouseup vmouseover vmouseout focus blur scrollstart": function (c) {
					...
					if (h.length) {
						...
						else if (j === "vmouseover" || j === "focus") i ? g = setTimeout(function () {
							h.removeClass("ui-btn-up-" + e).addClass("ui-btn-hover-" + e)
						}, b) : h.removeClass("ui-btn-up-" + e).addClass("ui-btn-hover-" + e);
						else if (j === "vmouseout" || j === "blur" || j === "scrollstart") h.removeClass("ui-btn-hover-" + e + " ui-btn-down-" + e).addClass("ui-btn-up-" + e), f && clearTimeout(f), g && clearTimeout(g)
					}
				},
	...
	function (a, b) {
		...
		a.widget("mobile.listview", a.mobile.widget, {
			...
			_removeCorners: function (a, b) {
				var c = "ui-corner-top ui-corner-tr ui-corner-tl",
		...
		a.widget("mobile.slider", a.mobile.widget, {
			...
			_create: function () {
				var e = this,
					...
					y = a(x),
					z = f.jqmData("highlight") && j !== "select" ? function () {
			...
			refresh: function (b, c, d) {
				...
				var e = this.element,
					...
					j = g === "input" && parseFloat(e.attr("step")) > 0 ? parseFloat(e.attr("step")) : 1;
				if (typeof b == "object") {
					var k = b,
						l = 8;
		...
		var e = function (d) {
			var e = d.select,
				...
				o = a("<ul>", {
					...
					role: "listbox",
					"aria-labelledby": k
				}).attr("data-" + a.mobile.ns + "theme", d.options.theme).appendTo(n),
				p = a("<div>", {
			...
			d.isMultiple && (t = a("<a>", {
				...
				build: function () {
					...
					b.refresh(), b.select.attr("tabindex", "-1").focus(function () {
						...
						var e = b.select[0].selectedIndex,
		...
		a.widget("mobile.fixedtoolbar", a.mobile.widget, {
				...
				e.closest(".ui-page").bind("pagebeforeshow", function () {
					...
					if (d.trackPersistentToolbars) {
						var g = a(".ui-footer-fixed:jqmData(id)", this),
							...
							j = h.length && f.nextPage && a(".ui-header-fixed:jqmData(id='" + h.jqmData("id") + "')", f.nextPage) || a();
			...
			_useTransition: function (c) {
				var d = a(b),
					...
					j = e.is(":jqmData(role='header')") ? "header" : "footer";
